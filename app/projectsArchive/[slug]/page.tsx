...
import { ArrowLeft, ExternalLink, Github, CheckCircle2, Lightbulb, Target, Laptop, X, ChevronLeft, ChevronRight, Menu } from 'lucide-react';
...
export default function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    getProject(resolvedParams.slug).then((data) => {
      setProject(data);
      setLoading(false);
    });
  }, [resolvedParams.slug]);
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex gap-8 items-center">
              <Link href="/projectsArchive" className="text-gray-600 hover:text-gray-900 transition-colors text-sm font-sans">
                Projects
              </Link>
              <Link href="/writings" className="text-gray-600 hover:text-gray-900 transition-colors text-sm font-sans">
                Writings
              </Link>
              <a href="/Shejal_Tiwari_Resume.pdf" target="_blank" className="text-gray-600 hover:text-gray-900 transition-colors text-sm font-sans">
                Resume
              </a>
              <Link href="/contact" className="px-5 py-2 bg-gray-900 hover:bg-gray-700 text-white rounded-md text-sm transition-colors font-sans">
                Contact
              </Link>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 md:py-16">
        ...
        <header className="max-w-4xl mb-16">
          ...
          <h1 className="text-4xl md:text-6xl font-serif font-normal text-gray-900 mb-8 leading-tight">
            {project.title}
          </h1>
          
          <p className="text-lg md:text-xl text-gray-600 leading-relaxed font-serif mb-10 max-w-3xl">
            {project.overview}
          </p>
        ...
        {project.images && project.images.length > 0 && (
          <section className="mb-20">
            ...
            <p className="mt-4 text-center text-[10px] text-gray-400 uppercase tracking-widest font-sans font-bold">
        ...
        <div className="grid md:grid-cols-2 gap-12 mb-20">
          {project.features && (
            <div>
              <h3 className="text-xl font-serif font-normal text-gray-900 mb-6 flex items-center gap-3">
                <CheckCircle2 size={20} /> Key Features
              </h3>
              <ul className="space-y-4 font-sans text-sm">
                {project.features.map((feat, i) => (
                  <li key={i} className="flex items-start gap-4 text-gray-600 leading-relaxed">
                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-gray-900 shrink-0" /> {feat}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {project.technicalHighlights && (
            <div>
              <h3 className="text-xl font-serif font-normal text-gray-900 mb-6 flex items-center gap-3">
                <Laptop size={20} /> Tech Highlights
              </h3>
              <ul className="space-y-4 font-sans text-sm">
                {project.technicalHighlights.map((high, i) => (
                  <li key={i} className="flex items-start gap-4 text-gray-600 leading-relaxed">
                    <span className="mt-2 h-1.5 w-1.5 rounded-full bg-gray-300 shrink-0" /> {high}
                  </li>
                ))}
              </ul>
            </div>
        ...
        {/* BODY CONTENT */}
        {project.body && (
          <article className="prose prose-lg max-w-none mb-20 prose-headings:font-serif prose-headings:font-normal prose-p:text-gray-600 prose-p:font-serif">
